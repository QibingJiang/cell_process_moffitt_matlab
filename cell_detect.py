...
def _1gaussian(x, amp1,cen1,sigma1):
    ...
    # print(arr)
    for data in arr:
        # print("qibing 36", data)
        if data < 0:
            print("qibing 39")
            return float("inf")

    # return amp1*(1/(sigma1*(np.sqrt(2*np.pi))))*(np.exp((-1.0/2.0)*(((x-cen1)/sigma1)**2)))
    tmp = amp1 * (np.exp((-1.0 / 2.0) * (((x - cen1) ** 2) / (sigma1 ** 2))))
    # print("qibing 44")
    # print("qibing 44", tmp)
    return tmp
...
# I don't have to use annulus to detect cells. So I thought maybe I just use 2 overlap contours to detect cells. Actually the performance is not good.
    ...
    def prepro_frames(self, path, pt, Beacon, para):
        ...
        #     print(frame_count, end=' ', flush=True)
            ...
            frame_count_str = para[2].format(frame_count)
            image_path = input_path + para[4] + frame_count_str + "_0_" + index_0 + index_1_str + para[3]
            ...
            frame = cv2.medianBlur(frame_org, 81)  # There is an unexpected effect when ksize is 81, applied to 8 times scaled image.
            ...
            if(frame_count == 0):
                out_image_path = out_path + "{0:0=1d}".format(frame_count) + ".tif"
                cv2.imwrite(out_image_path, frame)
                ...
                # self.background_pixel = np.argmax(frame_0_b_hist)
                ...
                # hi = plt.hist(frame.flatten(), 256, [0, 256], histtype='step', label = "{0:.2f}".format(self.background_pixel_mean) + " {0:.2f}".format(self.background_pixel_std))
                hi = plt.hist(frame.flatten(), 256, [0, 256], histtype='step', linewidth=2)
                self.background_pixel = np.argmax(hi[0])

                # self.background_pixel_peak = np.argmax(hi[0])
                # self.background_pixel = self.background_pixel_mean
                plt.plot(hi[1][self.background_pixel] + 0.05, hi[0][self.background_pixel], 'o')

                x_array = hi[1][0:-1]
                y_array_2gauss = hi[0]
                x_array = x_array.astype(int)
                y_array_2gauss = y_array_2gauss.astype(int)
                p0_guess = [hi[0][self.background_pixel], self.background_pixel_mean, 4]
                ...
                # plt.plot(x_array, _1gaussian(x_array, *popt_2gauss), 'k--', label="{0:.2f}".format(popt_2gauss[0]) + " {0:.2f}".format(popt_2gauss[1]) + " {0:.2f}".format(popt_2gauss[2]))
                plt.plot(x_array, _1gaussian(x_array, *popt_2gauss), '--', label="Estimated Gaussian", linewidth=2, color = (0.7, 0.3, 0.7))
                ...
                coordi_strr_2 = "[" + "{0}".format(int(hi[1][self.background_pixel])) + ", " + "{0}".format(int(
                    hi[0][self.background_pixel])) + "]"
                # plt.text(hi[1][self.background_pixel] + 0.05, hi[0][self.background_pixel], coordi_strr_2)
                plt.text(hi[1][self.background_pixel] + 0.05, hi[0][self.background_pixel], "Peak Pixel")
                ...
                debug = 0
            # if(frame_index == 0):
                frame = cv2.resize(frame, (frame.shape[1] * 8, frame.shape[0] * 8), interpolation=cv2.INTER_CUBIC)
                gray = frame
                ...
                for i in range(len(contours)):
                    try:
                        ...
                        for m in range(3):
                            for n in range(3):
                                idx_0 = contours[i][0][0][1] - 1 + m
                                idx_1 = contours[i][0][0][0] - 1 + n
                                ret = cv2.pointPolygonTest(contours[i], (idx_1, idx_0), False)
                        ...
                        if (ok == True):
                            ...
                            (x, y), radius = cv2.minEnclosingCircle(contours[i])
                            area = cv2.contourArea(contours[i])
                            cell_r_s.append([radius / 8, area / 64, np.mean(pixels)])
                        else:
                ...
                ###**** find 2 peaks ****#
                peaks, _ = find_peaks(hist_data[0], distance=15)
                hist_data = np.array([hist_data[0][peaks], hist_data[1][peaks]])
                hist_data[1] += 0.05
                max_loc_2 = np.argmax(hist_data[0])
                tmp = np.delete(hist_data[0], max_loc_2, 0)
                max_2nd_loc = np.argmax(tmp)
                if (max_2nd_loc >= max_loc_2):
                    max_2nd_loc += 1
                    p0_guess = [hist_data[0][0], hist_data[1][0], 0.5, hist_data[0][max_2nd_loc], hist_data[1][max_2nd_loc], 0.5]
                else:
                    p0_guess = [hist_data[0][0], hist_data[1][0], 0.5, hist_data[0][max_loc_2], hist_data[1][max_loc_2], 0.5]


                # tmp = np.array([hist_data[0][peaks], hist_data[1][peaks]])
                # max_loc_2 = np.argmax(tmp[0])
                # tmp[1] += 0.05
                # p0_guess = [hist_data[0][0], 0, 0.5, tmp[0][max_loc_2], tmp[1][max_loc_2], 0.5]

                # p0_guess = [hist_data[0][0], 0, 0.5, tmp[0][max_loc_2], tmp[1][max_loc_2], 0.5]

                print("guess: ", p0_guess)
                ...
                print(gg_fit)
                plt.plot(x_array, y_array_2gauss)
                ...
                plt.savefig(path + str(pt) + "_" + str(Beacon) + "_fit_2_peaks.png")
                ...
                # # For the beacon Pt180_87, the curve_fit works bad. What I input for the parameter is what I get from the output parameter. I think I cannot depend on the method. So I use the above method.
                # popt_2gauss, pcov_2gauss = scipy.optimize.curve_fit(_2gaussian, x_array, y_array_2gauss, p0 = p0_guess, maxfev = 50000)
                # print(p0_guess, "\n", popt_2gauss)
                #
                ...
                #
                # plt.plot(x_array, y_array_2gauss)
                # plt.scatter(x_array, y_array_2gauss, s = 8)
                # plt.plot(x_array, _2gaussian(x_array, *popt_2gauss), 'k--')
                #
                # # peak 1
                # # ax1.plot(x_array, gauss_peak_1, "g")
                # plt.fill_between(x_array, gauss_peak_1.min(), gauss_peak_1, alpha=0.5,
                #                  label="{0:.2f}".format(popt_2gauss[0]) + " {0:.2f}".format(
                #                      popt_2gauss[1]) + " {0:.2f}".format(popt_2gauss[2]))
                #
                # # peak 2
                # # ax1.plot(x_array, gauss_peak_2, "y")
                # plt.fill_between(x_array, gauss_peak_2.min(), gauss_peak_2, alpha=0.5,
                #                  label="{0:.2f}".format(popt_2gauss[3]) + " {0:.2f}".format(
                #                      popt_2gauss[4]) + " {0:.2f}".format(popt_2gauss[5]))


                # *** save hist ****** #
                # hist_data_cp = hist_data
                # tmp_len = len(hist_data_cp[0])
                # tmp_arr = np.array([hist_data_cp[1][0:tmp_len], hist_data_cp[0]])
                # tmp_arr = np.transpose(tmp_arr)
                # np.savetxt(path + str(pt) + "_" + str(beacon) + "_radius_hist.txt", tmp_arr, fmt="%.3f")
                ...
                # plt.plot(arr[max_loc][0], arr[max_loc][1], 'o')
                # coordi_strr = "[" + "{0:.2f}".format(arr[max_loc][0]) + ", " + "{0:.2f}".format(arr[max_loc][1]) + "]"
                # plt.text(arr[max_loc][0], arr[max_loc][1], coordi_strr)
                # plt.xlim(0,8)
                # plt.ylim(0,6000)
                # plt.xlabel("radius")
                # plt.ylabel("sum of white points area")


                # f_log = open(path + str(pt) + "_" + str(beacon) + "_log.txt", 'w')
                # print("The paras I guess: ", file = f_log)
                # print(p0_guess, file = f_log)
                # print("-------------Peak 1-------------", file = f_log)
                # print("amplitude = %0.2f (+/-) %0.2f" % (pars_1[0], perr_2gauss[0]), file = f_log)
                # print("center = %0.2f (+/-) %0.2f" % (pars_1[1], perr_2gauss[1]), file = f_log)
                # print("sigma = %0.2f (+/-) %0.2f" % (pars_1[2], perr_2gauss[2]), file = f_log)
                # print("area = %0.2f" % np.trapz(gauss_peak_1), file = f_log)
                # print("--------------------------------", file = f_log)
                # print("-------------Peak 2-------------", file = f_log)
                # print("amplitude = %0.2f (+/-) %0.2f" % (pars_2[0], perr_2gauss[3]), file = f_log)
                # print("center = %0.2f (+/-) %0.2f" % (pars_2[1], perr_2gauss[4]), file = f_log)
                # print("sigma = %0.2f (+/-) %0.2f" % (pars_2[2], perr_2gauss[5]), file = f_log)
                # print("area = %0.2f" % np.trapz(gauss_peak_2), file = f_log)
                # print("--------------------------------", file = f_log)
                #
                # f_log.close()

                # with open("/home/qibing/disk_t/" + str(pt) + "/curve_fit_ret.txt", "a") as f:
                #     if(popt_2gauss[1] < popt_2gauss[4]):
                #         np.savetxt(f, [np.concatenate(([beacon], popt_2gauss[0:3], popt_2gauss[3:6], [self.background_pixel, self.background_pixel_mean, self.background_pixel_std]))], fmt="%.2f")
                #         self.cell_core_r = popt_2gauss[4]
                #         self.cell_core_r_std = popt_2gauss[5]
                #     else:
                #         np.savetxt(f, [np.concatenate(([beacon], popt_2gauss[3:6], popt_2gauss[0:3], [self.background_pixel, self.background_pixel_mean, self.background_pixel_std]))], fmt="%.2f")
                #         self.cell_core_r = popt_2gauss[1]
                #         self.cell_core_r_std = popt_2gauss[2]

            else:
                print("prepro_frames image count: ", frame_count)
                image_a = image_b
                image_b = frame
    ...
    def prepro_frames_2(self, path, prepro_images_path):
        ...
        for frame_count in range(self.image_amount):
            ...
            if(frame_count == 0):
               ...
               #  print(cf.f_lineno))
                # print(_1gaussian, x_array, y_array_2gauss, p0_guess)
            ...
            # if(frame_index == 0):
               ...
               #  print(cf.f_lineno))
                ...
                print("\r", frame_count, end = "/" + image_amount_str)
                image_a = image_b
                image_b = frame
       ...
       #  print(cf.f_lineno))
        ...
        for i in range(self.image_amount):
            ...
            print("\r", i, end="/" + image_amount_str)
