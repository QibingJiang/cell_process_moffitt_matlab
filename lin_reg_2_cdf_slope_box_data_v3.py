...
# why this file is v3?
# In v2, I use max or min from the diff mat(ImageJ - CancerCellTracker)
# In v3, I only use max
...
# drugs = drug_sel = ['BORTEZOMIB','IXAZOMIB','PANOBINOSTAT']

# pt_sel = ['Pt174', 'Pt180', 'Pt181', 'Pt196', 'Pt210', 'Pt224', 'Pt230', 'Pt298_SOCCO', 'Pt400_SOCCO', 'Pt415_SOCCO'] # 
pt_sel = ['Pt317_SOCCO', 'Pt328_SOCCO', 'Pt338_SOCCO', 'Pt344_SOCCO', 'Pt350_SOCCO', "Pt352_SOCCO", 'Pt354_SOCCO', "Pt357_SOCCO", "Pt361_SOCCO", "Pt362_SOCCO"]# 
# pt_sel = ['Pt317_SOCCO', 'Pt328_SOCCO', 'Pt338_SOCCO']# 
...
# print(len(pt_sel). len(drugs))
# print(len(pt_sel). len(drugs))

box_plot = np.zeros([2, len(pt_sel), len(drugs)])
box_plot_max = np.zeros([len(pt_sel), len(drugs)])
...
########################################################################3 end
                ...
                for i in range(5):
                    ...
                    if(len(doses_imj[i]) == 0):
                        continue
                    else: #(len(doses_imj[i]) >= 1):
                        doses_imj[i][0] = doses_imj[i][0] / control_imj
                        if(len(doses_imj[i]) == 2):
                            doses_imj[i][1] = doses_imj[i][1] / control_imj
                ...
                for i in range(5):
                    if(len(doses[i]) == 0):
                        ...
                        doses[i][0] = np.where(control > 0, doses[i][0] / control, doses[i][0])
                        if(len(doses[i]) == 2):
                            doses[i][1] = np.where(control > 0, doses[i][1] / control, doses[i][1])
                ...
                # mean_0 = [mean(l[0]) for l in five_doses]
                # mean_0_imj = [mean(l[0]) for l in five_doses_imj]

                mean_0 = np.zeros(5)
                mean_0_imj = np.zeros(5)
                ...
                for box_i in range(5):
                    if(len(five_doses[box_i]) > 0 and len(five_doses_imj[box_i]) > 0): 
                        mean_0[box_i] = mean(five_doses[box_i][0])
                        mean_0_imj[box_i] = mean(five_doses_imj[box_i][0])
                        box_count += 1
                    
                    # if(len(doses[box_i]) > 0):
                    #     max_l.append(doses[box_i][0].max())
                    #     if(len(doses[box_i]) > 1):
                    #         max_l.append(doses[box_i][1].max())

                    # if(len(doses_imj[box_i]) > 0):
                    #     max_imj_l.append(doses_imj[box_i][0].max())
                    #     if(len(doses_imj[box_i]) > 1):
                    #         max_imj_l.append(doses_imj[box_i][1].max())



                    # print(doses_imj[box_i])
                    if(len(doses[box_i]) > 0):
                        diff = doses_imj[box_i][0] - doses[box_i][0]
                        # print(diff, doses_imj[box_i][0], doses[box_i][0])
                        # print(diff)
                        diff_l.append(diff)
                        if(len(doses[box_i]) > 1):
                            diff = doses_imj[box_i][1] - doses[box_i][1]
                            # print(diff, doses_imj[box_i][1], doses[box_i][1])
                            # print(diff)
                            diff_l.append(diff)



                mean_1 = sum(mean_0)/box_count
                mean_1_imj = sum(mean_0_imj)/box_count

                box_plot[0][pt_idx][drug_idx] = mean_1_imj - mean_1
                box_plot[1][pt_idx][drug_idx] = mean_0_imj[4] - mean_0[4]

                # print(diff_l)
                diff_np = np.array(diff_l)
                # print("diff_np: ", diff_np)
                max_diff = diff_np.max()
                # min_diff = diff_np.min()

                box_plot_max[pt_idx][drug_idx] = max_diff


                # if(max_diff >= -min_diff):
                #     box_plot_max[pt_idx][drug_idx] = max_diff
                # else:
                #     box_plot_max[pt_idx][drug_idx] = min_diff
                #     # print("qibing: ", box_plot_max[pt_idx][drug_idx], max_diff, min_diff)
...
################################################################33333333
        ...
        np.savetxt(out_path + "box_all_dose.txt", box_plot[0])
        np.savetxt(out_path + "box_least_dose.txt", box_plot[1])
        np.savetxt(out_path + "box_all_dose_max.txt", box_plot_max)
