...
precision = data[:, 1] / data[:, 0]
recall = data[:, 1] / ((data[:, 2] - data[:, 3]) + data[:, 1])
specificity = data[:, 3] / (data[:, 3] + data[:, 0] - data[:, 1])
accuracy = (data[:, 1] + data[:, 3]) / (data[:, 0] + data[:, 2])
f_1 = 2 * np.dot(precision, recall) / (precision + recall)

a = np.arange(len(precision))

plt.plot(a, precision, label = "precision")
plt.plot(a, recall, label = "recall")
plt.plot(a, specificity, label = "specificity")
